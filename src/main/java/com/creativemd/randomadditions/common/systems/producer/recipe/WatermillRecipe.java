...
public class WatermillRecipe extends ShapelessOreRecipe{

	public WatermillRecipe(ItemStack result, Object... recipe) {
		super(result, recipe);
	}
    ...
    public ItemStack getCraftingResult(InventoryCrafting var1){
		...
		for (int i = 0; i < var1.getSizeInventory(); i++) {
			...
			if(stack != null && RandomItem.watermillarm.isItem(stack))
			{
				if(length == 0)
		...
		output.stackTagCompound.setInteger("size", arms);
		if(error)
		{
			output.stackTagCompound.setInteger("length", 0);
			output.stackTagCompound.setInteger("size", 0);
		}
		return output;
    }
